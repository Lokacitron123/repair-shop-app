...
export default async function TicketFormPage({
  ...
  try {
    ...
    const [managerPermission, user] = await Promise.all([
      ...
      getUser(),
    ]); // await Promise.all helps us avoid waterfalls. We request both at the same time

    const isManager = managerPermission?.isGranted; // isManager is a boolean value
    ...
    if (ticketId) {
      ...
      if (isManager) {
        ...
        const techs = users
          ? users.map((user) => ({
              id: user.email?.toLowerCase() || "unknown",
              description: user.email?.toLowerCase() || "unknown",
            }))
          : [];
